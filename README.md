...
Epsilon-Greedy 是一種基礎的探索策略，在每一步中以機率 ε 隨機選擇一個 arm（探索），以機率 1−ε 選擇目前平均報酬最高的 arm（利用）。這種方法簡單直觀，易於實作，但若 ε 選得不好，可能在早期探索錯誤的 arm，導致後期無法改善。
...
### (1) Algorithm Formula (LaTeX)
UCB 方法基於「樂觀原則」，對每個 arm 計算其平均報酬與一個探索項（與選擇次數反比），選擇「上界最大」的 arm。這使得它在早期願意多試未被充分探索的 arm，但後期能聚焦在真正高報酬的 arm 上。UCB 適用於 arm 數量適中且對穩定性要求較高的場景。
...
### (1) Algorithm Formula (LaTeX)
Thompson Sampling 是一種貝葉斯策略，為每個 arm 建立報酬機率模型，每次根據模型進行隨機抽樣，選出最有可能是最佳的 arm。這種方法天然融合探索與 exploitation，隨著時間進展會越來越常選擇最佳 arm。其表現通常優於其他策略，尤其適用於需要快速學習、線上調整或樣本有限的情境。
